...
export function Login() {
  ...
  return (
    <div className="min-h-screen grid place-items-center bg-surface-light p-6">
      <div className="card w-full max-w-md p-8">
        ...
        <p className="text-sub-light text-sm mb-6">Learn the Amanotes metric tree in 5 minutes a day.</p>

        <form onSubmit={submit} className="space-y-3">
          <div>
            <label className="text-xs font-semibold uppercase text-sub-light">Display name</label>
            <input
              value={displayName}
              onChange={(e) => setDisplayName(e.target.value)}
              placeholder="Linh Nguyen"
              className="mt-1 w-full px-3 py-2 border border-slate-200 rounded-md focus:border-brand focus:outline-none"
            />
          </div>
          <div>
            <label className="text-xs font-semibold uppercase text-sub-light">Amanotes email</label>
            <input
              value={email}
