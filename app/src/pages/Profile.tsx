    ...
    <div className="max-w-3xl mx-auto px-4 md:px-6 pt-4 md:pt-6 pb-6">
      ...
      <section className="relative path-banner-business rounded-2xl p-5 text-white overflow-hidden shadow-[0_6px_0_0_rgba(0,0,0,0.15)]">
        <div className="flex items-center gap-4">
          <div className="w-20 h-20 rounded-2xl bg-white/20 grid place-items-center text-3xl font-black flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-1.5 mt-2">
              {user.role && <span className="pill bg-white/25 text-white">{user.role}</span>}
              {user.team && <span className="pill bg-white/25 text-white">{user.team}</span>}
            </div>
          </div>
        </div>
        <div className="mt-4 flex items-end justify-between">
          ...
          <div className="flex-1 mx-4">
            <div className="h-2 bg-white/25 rounded-full overflow-hidden">
              <div className="h-full bg-white" style={{ width: `${lvl.pct * 100}%` }} />
            </div>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
        <KPI label="Streak" value={`${streak.current}d`} icon="🔥" tint="#F79009" tintBg="#FFF1D6" />
        <KPI label="Longest" value={`${streak.longest}d`} icon="📈" tint="#19a06b" tintBg="#D7FFB8" />
        <KPI label="Hearts" value={`${hearts.count}/5`} icon="❤" tint="#E92C2C" tintBg="#FFE5E5" />
        <KPI label="Freezes" value={String(streak.freezes)} icon="🧊" tint="#1899D6" tintBg="#DDF4FF" />
      </section>

      {/* Path progress */}
      <section className="surface mt-4 p-5">
        <h2 className="text-sm uppercase tracking-wider font-black text-sub-light mb-4">Path progress</h2>
            ...
            return (
              <div key={p.slug}>
                <div className="flex items-center justify-between text-sm mb-1.5">
                  ...
                  <span className="text-xs text-sub-light font-mono">{done}/{total} · {pct}%</span>
                </div>
      ...
      {/* Settings */}
      <section className="surface mt-4 p-5">
        <h2 className="text-sm uppercase tracking-wider font-black text-sub-light mb-3">Settings</h2>
  ...
  return (
    <div className="surface p-3 flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[10px] uppercase tracking-wider font-bold text-sub-light mt-1">{label}</div>
      </div>
    </div>
  )
}
...
function ToggleRow({ label, on, onChange }: { label: string; on: boolean; onChange: (v: boolean) => void }) {
  return (
    ...
    >
      ...
      <span
        className={`w-12 h-7 rounded-full p-0.5 transition-colors ${on ? 'bg-[#1CB0F6]' : 'bg-slate-300'}`}
        aria-hidden
      >
        <span
