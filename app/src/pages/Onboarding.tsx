  ...
  return (
    <div className="min-h-screen bg-bg-0 p-6">
      <div className="max-w-3xl mx-auto pt-10">
        <div className="text-xs text-fgmuted mb-1">Step 1 of 2</div>
        <h1 className="text-2xl font-extrabold mb-2">Welcome to DuoData. What's your role?</h1>
        <p className="text-fgmuted mb-6">We use this to personalize your starting path and tailor lesson examples.</p>
          ...
          {ROLES.map((r) => (
            <button
              ...
              className={`card p-4 text-left transition-colors ${
                role === r.code ? 'border-brand ring-2 ring-brand/30' : 'hover:border-linestrong'
              }`}
            >
              <div className="font-bold">{r.name}</div>
              <div className="text-xs text-fgmuted mt-1">{r.hint}</div>
            </button>
          ))}
        </div>

        <div className="card p-5 mt-6">
          <label className="text-xs font-semibold uppercase text-fgmuted">Team / product</label>
          <select
            value={team}
            onChange={(e) => setTeam(e.target.value as Team)}
            className="mt-2 w-full md:w-72 px-3 py-2 border border-line rounded-md focus:border-brand focus:outline-none"
          >
            {TEAMS.map((t) => (
              <option key={t} value={t}>{t}</option>
            ))}
          </select>
          <p className="text-xs text-fgmuted mt-2">PI · BH · DR · GDUC = LO4 games · NGD = New Games · MEP = Music Entertainment Platform · GS · DnI · Other</p>
        </div>
